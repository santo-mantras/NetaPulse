...
def scrape_up_fixed():
    ...
    for t in tables:
        ...
        if 'District' in headers and 'Constituency' in headers and len(t.find_all('tr')) > 300:
            target_table = t
            break
    ...
    party_map = {
        ...
        'ADS': 'Apna Dal (Sonelal)',
        'NISHAD': 'NISHAD Party',
        'JSD(L)': 'Jansatta Dal (Loktantrik)',
        'IND': 'Independent'
    }
        ...
        try:
            ...
            if candidate_name and const_name:
                data.append({
                    ...
                    "constituency_code": f"AC-UP-{const_no}",
    ...
    print(f"Scraped {len(data)} UP constituencies with ACTUAL winner names!")
    for s in data[:5]:
...
if __name__ == "__main__":
    res = scrape_up_fixed()
    with open("scripts/pipeline/up_scraped_403.json", "w", encoding="utf-8") as f:
