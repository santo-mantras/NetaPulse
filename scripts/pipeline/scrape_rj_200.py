...
def scrape_rajasthan_official():
    ...
    # Table 8 is the official constituency results table
    table = soup.find_all('table', {'class': 'wikitable'})[8]
    
    current_district = "Sri Ganganagar"
    results = []
    ...
    for tr in rows[2:]:
        ...
        results.append({
            ...
            'constituency_code': f"AC-RJ-{cno}",
            'constituency_name': cname,
            'winner': cand_name,
            'party': party_full
        })

    print(f"Scraped {len(results)} Rajasthan assembly constituencies!")
...
if __name__ == "__main__":
    res = scrape_rajasthan_official()
    with open("scripts/pipeline/rj_scraped_200.json", "w", encoding="utf-8") as f:
